...
var info = manager.PluginInfo{
	Name: "权限鉴权",
	Usage: `
部分插件会有群管理员权限等级要求，即在帮助中以方括号包裹的数字，只有高于该权限等级的群管理员才能使用
本插件用于进行群管理员权限等级设置
用法：
	更新管理员权限：会将所有群中未被设置权限的管理员设为默认权限等级（管理员5、群主1）
...
配置项：
	部分插件已预置了最低权限等级，但所有插件皆可通过config-plugin问句进行权限等级配置
	只需配置该文件中的 插件名.adminlevel 配置项，即可设置指定插件的最低权限等级了
